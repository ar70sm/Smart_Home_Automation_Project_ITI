...
u8 features [3]={0,0,0};         // {features(2),features(3),features(4)}
								 //  {LIGHT     ,DOOR       ,TV         }
...
/* ******************************************** THE MAIN FUNCTION ****************************************** */
void THE_MAIN_FUNCTION (void){
	u8 getVal = ADC_u16Read(ADC0);     // convert value from analog to digital
	if (value != getVal){
		value =getVal;
		temp =step*value*7.114;        // The value of temperature == 0.06836 * value * 7.114
		HCLCD_vidClear();
		HCLCD_vidSendString("TEMP: ",0);
		HCLCD_vidSendFloat(temp,2);
		/* ********************** ALARME ********************** */
	...
	ADJ_BIT(PORTA,6,features[2]); // TV OUTPUT
